...
func (src Source64) Uint64() uint64 {
	if src.ubit == FINAL_INT64 {
		src.ubit = uint64(src.src.Int63()) | 1
	}
	src.ubit <<= 1
	return uint64(src.src.Int63()) ^ src.ubit
}
...
func randomBytes(src Source64, out chan<- []byte) {
	var (
		r uint64
		i = BSIZE
	)
...
func main() {
	...
	for i := 0; i < SOURCES; i++ {
		...
		src := Source64{rand.NewSource(seed), FINAL_INT64}
